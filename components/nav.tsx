...
function getReloadConfig(
  pathname: string,
  mode: "google" | "demo",
): { scope: FinanceDataScope; idleLabel: string; loadingLabel: string } {
  if (mode === "demo") {
    return {
      scope: "all",
...
export function Nav() {
  ...
  const reloadLabel = isReloading
    ? reloadConfig.loadingLabel
    : reloadConfig.idleLabel;
